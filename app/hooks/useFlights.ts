...
const useFlights = () => {
  ...
  const setIsFetching = (isFetching: boolean) => dispatch({ type: FlightsDispatchActionType.fetchingFlights, payload: { isFetching } });
  const setFlightsData = (flights: Flight[] | undefined) => dispatch({ type: FlightsDispatchActionType.successFetchingFlights, payload: { flights } });
  const setFetchingError = (error: Error | undefined) => dispatch({ type: FlightsDispatchActionType.errorFetchingFlights, payload: { error: error } });
  ...
  const sortFlights = useCallback((direction: SortDirection) => {
    const { flights } = state
